---
icon: octicons/list-ordered-24
description: Instructions for building and setting up the 3MS modular multimaterial system for Klipper 3D printers.
comments: true
---

<link rel="stylesheet" href="../assets/css/badges.css">

<style>
.inst-button {
    width: 35ch;
    text-align: center;
}
</style>

# Master Instructions

Follow this guide to assemble your 3MS. A special thank you if you purchased a kit!

Feel free to ask on the 3DCoded Discord any time if you need help during your build!

[![Discord Shield](https://discord.com/api/guilds/1307104511663411210/widget.png?style=banner2)](https://discord.gg/ekqxDhdGCg)

## Chapters

[**:material-cart: Getting a BOM**](bom.md){.inst-button .md-button}

[**:material-tools: Printed Parts and Tools**](printed-parts-and-tools.md){.inst-button .md-button}

[**:octicons-gear-24: Hardware Assembly**](hardware-assembly.md){.inst-button .md-button}

[**:fontawesome-solid-microchip: Electronic Assembly**](electronic-assembly.md){.inst-button .md-button}

[**:material-puzzle: Final Assembly**](final-assembly.md){.inst-button .md-button}

[**:material-console: Software Setup**](software-setup.md){.inst-button .md-button}

[**:material-code-json: Configuration**](config.md){.inst-button .md-button}

[**:fontawesome-solid-ruler: Calibration**](calibration.md){.inst-button .md-button}

[**:fontawesome-solid-flag-checkered: Slicer Setup and First Print**](firstprint.md){.inst-button .md-button}